...
def get_grid_index(grid_boundaries, record):
    for i in range(len(grid_boundaries)):
        ...
        if record[tweet_field_index['lat']] >= min_lat and record[tweet_field_index['lat']] < max_lat:
            if record[tweet_field_index['lon']] >= min_lon and record[tweet_field_index['lon']] < max_lon:
                return i
